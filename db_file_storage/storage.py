...
"""
    ...
    Check here: https://github.com/django/django/blob/1.7/django/db/models/loading.py
"""
...
def _get_unique_filename(model_class, filename_field, filename):
    final_name = filename
    stem, extension = final_name.rsplit('.', 1) if ('.' in final_name) else (final_name, '')
    append_to_name = 1
...
class DatabaseFileStorage(Storage):
    def _open(self, name, mode='rb'):
        ...
        model_instance = model_class.objects.only(content_field, mimetype_field).get(**{filename_field: name})
    ...
    def exists(self, name):
        model_class_path, content_field, filename_field, mimetype_field, filename = name.split('/')
        model_class = _get_model_class(model_class_path)
        return model_class.objects.filter(**{filename_field: filename}).exists()
